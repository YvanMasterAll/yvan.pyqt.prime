...
TEMPLATE_QRC_FILE = '    <file>skin/{fname}</file>'
...
def create_images(base_svg_path=SVG_PATH, rc_path=RC_PATH,
                  palette=DarkPalette):
    """Create resources `skin` png image files from base svg files and palette.
    ...
    # Get skin links from scss to check matches
...
def get_rc_links_from_scss(pattern=r"\/.*\.png"):
    """
    Get all skin links from scss file returning the list of unique links.
