...
class CPaginationBar(QWidget):
    ...
    def setTotalPages(self, totalPages):
        ...
        layout.addItem(QSpacerItem(
            0, 20, QSizePolicy.Fixed, QSizePolicy.Minimum))
        # 添加上一页按钮
    ...
    def setupUi(self):
        ...
        self.buttonPrevious = QPushButton(
            qtawesome.icon('mdi.chevron-left', color=ResourceLoader().qt_color_text), '', self, enabled=False, visible=False, clicked=self._doPageTurning,
            objectName='CPaginationBar_buttonPrevious')
        self.buttonPrevious.setIconSize(QSize(18, 18))
        # 下一页按钮
        self.buttonNext = QPushButton(
            qtawesome.icon('mdi.chevron-right', color=ResourceLoader().qt_color_text), '', self, enabled=False, visible=False, clicked=self._doPageTurning,
